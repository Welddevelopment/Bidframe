...
`source_rect` (additive, nullable — J-049 P3) is the PDF bounding box(es) of `source_excerpt` on `source_page`: `[[x0, y0, x1, y1], …]` in PDF points (top-left origin), one rect per line of a multi-line excerpt. Backend fills it via PyMuPDF `page.search_for` at extract time; absent when the excerpt can't be located verbatim (reflowed/OCR'd text), so the source panel falls back to a client-side text-layer search. Lets the verification view highlight the exact line instead of guessing.
