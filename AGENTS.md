...
- **Refreshed automatically** on every push to `main` by `.github/workflows/codemap.yml` — no matter who
  pushed. Do **not** hand-edit `CODEMAP.md`; the generator overwrites it.
- **You keep it current too:** if you add files or change structure, regenerate it in the *same* commit
...
- **Keep the map current.** If your change **adds/renames/deletes a file or alters structure** (new
  ...
  refreshed `CODEMAP.md` **in the same change**. CI also refreshes it on push, but don't lean on that —
  keep your own commits self-consistent.
