...
def test_get_round_trips_answers_and_caps(client):
    ...
    store.save_tender(TenderResponse(tender_id=tid, title="RT", requirements=enriched,
                                     capability_docs=caps), filename="rt.pdf")

    r = client.get(f"/tenders/{tid}/requirements")
...
def test_draft_endpoint_enriches_and_persists(client):
    ...
    # before: no answer
    assert client.get(f"/tenders/{tid}/requirements").json()["requirements"][0]["answer"] is None

    resp = client.post(f"/tenders/{tid}/draft", params={"provider": "mock"})
    ...
    after = client.get(f"/tenders/{tid}/requirements").json()
