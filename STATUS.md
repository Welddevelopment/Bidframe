...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **all 4 lane items shipped** — `engine/` on main (**79 tests**): reconcile **live in `backend/pipeline.py`** (conservative dedupe + noisy-OR + safety escalation); `needs_review` **calibrated 0.70** (LLM confidence weakly informative, G-007); **eval harness + aggregate `eval_all`** (SPSO **recall ~0.9 / gating recall 1.0 / 0 dangerous**); **auditable autofill** = grounded answer-draft + gap interview, mock+OpenAI, **never bluffs** (G-008); 66pp tender survives extract→reconcile (robustness) | @all: finish handoff gold; **@backend: capability-doc ingest + wire autofill into the live API**; @j: render.yaml deploy + **demo must use OpenAI** (heuristic=0 gating recall) | not blocked |
