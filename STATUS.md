...
| **Frontend** | compliance matrix · source panel · decision controls · graph view · demo | 🟢 **wireframes live + deployed + rebranded** — UI renamed to **Bidframe**; separate routes + persistent navbar, source+decision **slide-over drawer** (approve/edit/flag), **gating hero**, **React Flow** relationship graph (criteria_ref + depends_on), **upload** flow; in-memory decision state survives navigation; **autofill schema mirrored** (`answer`/`open_questions`/`capability_docs`) + worked mocks; live on Vercel | Answer + evidence panel & gap-interview UI (Day 3–4); capability-doc upload mode; design-system pass; swap to real `/requirements` when backend mock is up | backend mock `/requirements` for real-data swap |
