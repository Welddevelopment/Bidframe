...
| **Frontend** | compliance matrix · source panel · decision controls · graph view · demo | 🟢 **autofill UI live** — Bidframe wireframes (matrix, slide-over drawer, gating hero, React Flow graph, upload) **+ answer + evidence panel** (drafted answers with capability-doc citations, visual confidence, inline edit) **+ gap-interview** to-do list at `/answers`; in-memory state survives nav; deployed on Vercel | Design-system pass (ideating separately); swap to live `/requirements` (backend API is up — see `frontend-integration.md`); capability-doc upload mode | nothing blocking |
