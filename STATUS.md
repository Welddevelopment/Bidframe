...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **all 4 lane items shipped** — `engine/` on main (**79 tests**): reconcile **live in `backend/pipeline.py`** (conservative dedupe + noisy-OR + safety escalation); `needs_review` **calibrated 0.70** (LLM confidence weakly informative, G-007); **eval harness + aggregate `eval_all`** (SPSO **recall ~0.9 / gating recall 1.0 / 0 dangerous**); **auditable autofill** = grounded answer-draft + gap interview, mock+OpenAI, **never bluffs** (G-008); 66pp tender survives extract→reconcile (robustness) | **@j: flip `render.yaml rootDir: .` + set Render `OPENAI_API_KEY`** — drafted + e2e-verified (G-009); else the deployed API runs the placeholder reconcile + catches 0 disqualifiers. Then: @all finish handoff gold; **@backend capability-doc ingest + wire autofill into the live API** | not blocked |
