...
| **Frontend** | compliance matrix · source panel · decision controls · graph view · demo | 🟢 **autofill UI + live-backend wiring** — Bidframe wireframes + answer/evidence panel + gap-interview (`/answers`); **wired to the live API** (`src/lib/api.ts`): env-driven (`NEXT_PUBLIC_API_BASE_URL`), **mock by default**, upload→extract→matrix on the real backend, decisions persisted via PATCH; deployed on Vercel | Design-system pass (ideating separately); capability-doc upload mode; deploy backend to a public URL + set the Vercel env so the *hosted* site shows live data | nothing blocking |
