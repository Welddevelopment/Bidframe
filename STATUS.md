...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **engine LIVE in the pipeline** — `engine/` on main (67 tests green); **wired into `backend/app/pipeline.py`** (conservative dedupe + noisy-OR + safety escalation + `needs_review`; OpenAI path = healthy 9% flagged), import-safe fallback (G-005). SPSO eval: **recall ~0.9, gating recall 1.0, gating accuracy 1.0, 0 dangerous misses** | @j: deploy `render.yaml` so engine is live on Render too (G-005); confidence calibration + answer-draft (Day 3) | not blocked |
