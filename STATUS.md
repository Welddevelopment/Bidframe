...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **all 4 lane items shipped + autofill LIVE, fast & proven** — `engine/` on main (**116 tests**): reconcile **live in `backend/pipeline.py`** (conservative dedupe + noisy-OR + safety escalation); `needs_review` **calibrated 0.70** (G-007); **eval harness + aggregate `eval_all`** (SPSO **recall ~0.9 / gating recall 1.0 / 0 dangerous**); **auditable autofill wired into the live API** — `GET /requirements` returns grounded answers + gaps (mock on upload), `POST /tenders/{id}/draft` (OpenAI, **parallelized + `?limit`**, capability-doc upload); **groundedness eval proves 0 bluffs** (SPSO 32 grounded / 96 gaps / 42-42 citations verified); **sharper gap questions** via J's prompt (G-010..G-012); 66pp tender survives extract→reconcile; **adversarial trust-invariant suite** — the 4 trust claims hold under attack (G-015) | **@j: flip `render.yaml rootDir: .` + set Render `OPENAI_API_KEY`** — drafted + e2e-verified (G-009); else the deployed API runs the placeholder reconcile + catches 0 disqualifiers. Then: @all finish handoff gold; **@backend capability-doc ingest + wire autofill into the live API** | not blocked |
...
| **J** | prompts · orchestration · narrative · traction · glue | 🟢 **ahead** — 4 prompts (**gating fix verified, G-004**), raw-extraction contract, autofill scope+schema, comms, standup; scaffolded + **deployed the backend to Render**; **traction dossier DONE** (`traction-research.md`: 19 CRM targets, 14 verified handles, every sector + outreach kit); **demo narrative updated to the locked numbers** | **LLM-key plan** (organisers pushed for credits; **pre-bake demo via @generalist, J-020**; Claude = live fallback); **start traction outreach** (dossier ready, build now demo-solid); lock 90-sec script + rehearse; Fetch.ai go/no-go; decide the G-009 deploy flip (key-gated) | nothing |
