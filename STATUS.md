...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **engine LIVE + aggregate harness** — `engine/` on main (70 tests green); **wired into `backend/app/pipeline.py`** (conservative dedupe + noisy-OR + safety escalation + `needs_review`; OpenAI = 9% honest flag), import-safe fallback. SPSO eval **recall ~0.9 / gating recall 1.0 / 0 dangerous**; **aggregate `eval_all` across all gold** (eval-manifest); **66pp tender survives extract→reconcile, no crash** (35% robustness). G-005/G-006 | @all: finish handoff gold (museum); @j: render.yaml deploy + **demo must use OpenAI** (heuristic=0 gating recall); confidence calibration + answer-draft (Day 3) | not blocked |
