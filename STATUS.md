...
| **Generalist** | reconcile/dedupe · confidence routing · eval harness · answer-draft | 🟢 **REAL number on SPSO** — `engine/` on main, 64 tests green; PDF→extract→reconcile→eval loop scores SPSO pp.1-6 at **recall ~0.9 (17-18/19, run-to-run), gating recall 1.0, 0 dangerous misses**; **gating accuracy 0.39→1.0 after J's prompt fix (J-019 verified, G-004)** | Triage SPSO false positives (gold completeness vs noise); confidence calibration + answer-draft (Day 3) | not blocked |
