...
only deriving from current row decisions. Mock/frozen screens keep the derived fallback, so `/showcase`
and `/pack` still work offline. `ProcessingView` also renders the backend `docs` job snapshot as a per-file
tender-pack progress list with format badges and per-document stage labels. Verified `npm run lint` green
with the existing TanStack Virtual warning and `npm run build` green after allowing Google Fonts.
