...
- First 15 seconds must land the missed-deal-breaker pain.
- `/showcase` is the live product surface. Do not use `/demo` for row clicks; it is read-only.
- Show control live: ControlPanel, source proof, answer citation, open question, and approve-with-confirm if rehearsed.
...
- If browser/animation fails: use deck PDF/screenshots plus spoken source-proof narration.
- If asked for a live upload: "After the pitch, yes. On stage we use cached output of a real pipeline run so the measured result is repeatable."
- Claim ledger should be open in a tab for Q&A.
